...
Текущая ревизия исправляет runtime-зацикливание `find_in_page` и ошибочное требование строгой verification после focus/search click. Реальные Chromium, ChromeDriver и synthetic CDP проверены; точные результаты — в `TEST_REPORT.md`. После первоначальной сетевой ошибки `ERR_EMPTY_RESPONSE` автономный Wikipedia E2E был повторён с реальным OpenAI-compatible LLM и успешно завершён через видимый интерфейс: агент самостоятельно использовал поиск, пережил stale-ref/recovery и вернул 1137 с фактическим URL статьи. Реальный LLM tool calling также проверен через provider health check. Synthetic shop LLM acceptance выполнялся до взаимодействия с корзиной/verification, но полный сценарий не засчитывается как PASS из-за внешней ошибки provider HTTP 413. Реальные авторизованные аккаунты остаются непроверенными.
...
`find_in_page` и `extract_collection` не вызывают промежуточный observe: новые refs добавляются в текущую generation и доступны следующему decision. Расширение списка refs само по себе не сбрасывает search budget. После ошибки нужен fresh observation. Перед действием и после confirmation проверяются identity элемента, форма/её action, autocomplete и принадлежность вкладке. Изменение несвязанного DOM не инвалидирует стабильный control. Возврат ref не гарантирует, что элемент не изменится асинхронно: в этом случае возвращается `StaleRef` и структурированная причина recovery.
